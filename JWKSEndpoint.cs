...
static class JWKSEndpoint
{
    public static void Setup(WebApplication app, RsaSecurityKey publicKey, JsonSerializerOptions jsonOptions)
    {
        ...
        // kid: "lab-key-1" - thisis the key id, used by the API to look up which key to use to verify when a token arrives (tokens will have a matching kid in their header)
