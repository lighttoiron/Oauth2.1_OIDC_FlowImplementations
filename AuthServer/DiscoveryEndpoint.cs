    ...
    public static void Map(WebApplication app, JsonSerializerOptions jsonOptions)
    {
        app.MapGet("/.well-known/openid-configuration", (IOptions<AuthServerOptions> options) =>
        {
            ...
            var doc = new
            {
                ...
                scopes_supported = new[] { "openid", "profile", "email", "offline_access" }, // A list of all supported scopes that can be requested by clients. Scopes are used to specify the level of access that the client is requesting from the user.
